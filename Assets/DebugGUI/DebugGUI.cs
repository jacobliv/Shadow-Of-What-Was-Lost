    ...
    class GraphContainer
    {
        ...
        public void Push(float val) {
            ...
            if (currentIndex >= values.Length) {
                ...
                float[] nArray = new float[values.Length];
                Array.Copy(values,1,nArray,0,values.Length-1);
                nArray[currentIndex] = val;
                values = nArray;
                end = true;
            }
            else {
            ...
            if (end) { // if at the width, then shift graph to left
                Graphics.CopyTexture(
                    source, 0, 0, 1, 0, source.width - 2, source.height,
                    target, 0, 0, 0, 0
                );
            }
            else { // if not at width, keeping appendng
                Graphics.CopyTexture(
                    source, 0, 0, 0, 0, source.width - 1, source.height,
                    target, 0, 0, 0, 0
                );
            ...
            // for (int i = 0; i < target.height; i++)
            // {
            //     target.SetPixel(values.Length, i, Color.clear);
            // }

            // Read from index backwards
            var prevValue = values[currentIndex-1];
            var nextVal = values[currentIndex];

            // Flip the y coordinate to start at the bottom
            int y0 = (int)(Mathf.InverseLerp(min, max, prevValue) * graphHeight);
            int y1 = (int)(Mathf.InverseLerp(min, max, nextVal) * graphHeight);
            ...
            DrawLine(target, currentIndex-1, y0, currentIndex, y1, color);
        }
